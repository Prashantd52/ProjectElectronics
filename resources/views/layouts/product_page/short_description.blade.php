            ...
            <div class="prd-block_info prd-block_info--style1" data-prd-handle="/products/copy-of-suede-leather-mini-skirt">
              <div class="prd-block_info-top prd-block_info_item order-0 order-md-2">
                <div class="prd-block_price prd-block_price--style2">
                  <div class="prd-block_price--actual">${{$product->min_price +0}}</div>
              ...
              <div class="prd-block_info_item prd-block_info-when-arrives d-none" data-when-arrives>
                <div class="prd-block_links prd-block_links m-0 d-inline-flex">
                  ...
                  <div><a href="#" data-follow-up="" data-name="{{$product->name}}" class="prd-in-stock" data-src="#whenArrives">Inform me when the item arrives</a></div>
                </div>
              </div>
              <div class="prd-block_info-box prd-block_info_item">
                <div class="two-column">
                  <p>Availability:
                    <span class="prd-in-stock" data-stock-status="">@if($product->stock_quantity > 0)In stock @else Out of Stock @endif</span>
                  </p>
                  ...
                  <div class="prd-block_actions prd-block_actions--wishlist">
                    <div class="prd-block_qty">
                      <div class="qty qty-changer">
                        ...
                        <input type="number" class="qty-input" id="quantity{{$product->id}}" name="quantity" value="1" data-min="1" data-max="{{$product->stock_quantity}}">
                    ...
                    <div class="btn-wrap">
                    <button class="btn js-prd-addtocart js-trigger-addtocart js-prd-addtocart" type="submit" >Add To Cart</button> -->
                      @if($product->stock_quantity > 0)
                        <button class="btn btn--add-to-cart js-trigger-addtocart js-prd-addtocart"  onclick="addToCartWithQuantity({{$product->id}})" data-product='{"name":  "{{$product->name}}",  "path ": "{{$img_url}}{{$product->img_path}}", "url ": "{{route('electronic.product',$product->slug)}}",    "aspect_ratio ": "0.78"}' >Add to cart</button>
                      @else
                        <button class="btn btn--add-to-cart" disabled title="Out of Stock">Add to cart</button>
                      @endif
                    </div>
                    <div class="btn-wishlist-wrap">
                        @if($wished== 1)
                        <a href="#" onclick="remove_from_wishlist({{$product->id}})" class="btn-add-to-wishlist ml-auto btn-add-to-wishlist--add js-add-wishlist" title="Remove From Wishlist"><i class="icon-heart-hover"></i></a>
                        <a href="#" onclick="add_to_wishlist({{$product->id}},{{$product->product_id}})" class="btn-add-to-wishlist ml-auto btn-add-to-wishlist--off js-remove-wishlist"  title="Add To Wishlist"><i class="icon-heart-stroke"></i></a>
                        @else
                            <a href="#" onclick="add_to_wishlist({{$product->id}},{{$product->product_id}})" class="btn-add-to-wishlist ml-auto btn-add-to-wishlist--add js-add-wishlist"  title="Add To Wishlist"><i class="icon-heart-stroke"></i></a>
                            <a href="#" onclick="remove_from_wishlist({{$product->id}})" class="btn-add-to-wishlist ml-auto btn-add-to-wishlist--off js-remove-wishlist" title="Remove From Wishlist"><i class="icon-heart-hover"></i></a>
                        @endif
